 ...
 * failure (0) or the load balancer / gateway being unavailable. These send the
 * user to the dedicated connection-error page instead of a transient modal.
 */
const CONNECTION_ERRORS = new Set([0, 502, 503, 504]);

 // Requests that show their OWN inline error (so no global modal on failure).
 // The login screen renders auth failures inline, so suppress the modal there.
